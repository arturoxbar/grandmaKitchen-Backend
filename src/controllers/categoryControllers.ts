...
const getUserCategories = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const categories = await Category.find({ owner: req.user._id }).populate("owner", "username email");;
        return res.status(200).json({ categories });
    } catch (error) {
...
const assignCategoryToRecipe = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        const existingRelation = await CategoryRecipe.findOne({ recipeId, categoryId });
        if (existingRelation) {
            return res.status(400).json({ message: "La categoría ya está asignada a la receta" });
        }
        const newRelation = await CategoryRecipe.create({ recipeId, categoryId });
        return res.status(201).json({ message: "Categoría asignada a la receta", relation: newRelation });
    } catch (error) {
